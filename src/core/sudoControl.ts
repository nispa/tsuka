...
export function controlSudo(permissions: PermissionManager, argument: string): string {
  ...
  return permissions.isSudo()
    ? 'SUDO ON — all shell commands are authorized for all agents in this session, regardless of role or model tier. OS privileges are unchanged. Use /sudo off to revoke.'
    : 'SUDO OFF — normal command visibility and approval rules apply. Use /sudo on to enable.';
}
