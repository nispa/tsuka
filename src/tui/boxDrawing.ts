...
/**
 * How a pane frame is drawn. Absent means the classic thin rounded box; `lcars`
 * draws an LCARS elbow in `color` (hex). Both keep the exact same geometry — one
 * row above and below the content, one column on each side — because the mouse
 * hit-testing and the scrollbar maths are built on that box, not on its looks.
 */
