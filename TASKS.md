...
- **Test Suite Totali**: **116 suite automatizzate** al 2026-09-26 (numero reale: ultima riga di `npm test`)
- **Fase Attuale**: **Fase 11 — Seguiti della verifica di sicurezza e della TUI (T24)**. Fase 10 chiusa: T23.1–T23.16 fatti, T23.8 mitigato (processo confinato; il permission model di Node non protegge da codice malevolo).
...
- [Fase 10 — Audit-Driven Security & Reliability Hardening](#fase-10--audit-driven-security--reliability-hardening) (T23.1 – T23.16)
- [Fase 11 — Seguiti della verifica di sicurezza e della TUI](#fase-11--seguiti-della-verifica-di-sicurezza-e-della-tui-t24) (T24.1 – T24.19)
...
| T22.17 | ✅ Fatto | **Documentazione verificata**: scritta la Tappa 12 delle guide IT/EN (sub-agenti e delega del contesto): meccanismo, contratti `TaskPacket`/`AgentResult`, politica a due soglie, regole di sicurezza e limiti (il padre non si alleggerisce; il figlio riparte dall'obiettivo e dallo stato del workspace). Descrive il comportamento coperto dai test; dopo il checkpoint A (T24.3) la sezione 12.7 riporta i risultati misurati e la decisione di tenere spenta la delega automatica. |
...
| T24.3 | ✅ Fatto | **Checkpoint A eseguito** (26/09/2026, `scripts/checkpoint-a.ts`, 3 task × spento/acceso × 2, agente neutro): con finestre realistiche (≥20k) la delega non scatta (picco 0,65); forzata su 12k con Ornith 9B non migliora gli esiti (B: 1/2 sia spento sia acceso; C: deleghe fallite, turni 2–3× più lunghi) e costa un'amplificazione da 90 a oltre 1.000. Decisione: delega automatica spenta di default, `spawn_agent` resta. Nessun limite di memoria emerso: T22.12, T22.14, T22.15 restano rinviati. Trovati e corretti la potatura senza margine (T24.18) e il conteggio token del runner. |
| T24.4 | ✅ Fatto | **Rendering della memoria fuori dal backend**: il contratto `MemoryBackend` perde `formatForPrompt`/`formatRelevant` e `forgetFact` e guadagna `selectForPrompt(limit, sources) → { facts, available }`; tetto `memoryMaxChars` e formattazione vivono una volta sola nel facade `MemoryStore` (la rilevanza riusa `search` con `touch: false`). Test: un backend plug-in senza alcuna logica di budget produce comunque una sezione entro il tetto (MB5e2). |
...
| T24.8 | ✅ Fatto | **Igiene di TASKS.md e AGENTS.md**: tolta la riga T19.3 duplicata; intestazione del dashboard allineata (116 suite, fase 11, fase 10 chiusa), indice delle fasi esteso a T23.16 e alla fase 11. In `AGENTS.md` una sola riga di metriche datata e con il comando per ricontarle; tolti i numeri che invecchiano da direttiva 6, indice dei file e cheatsheet. |
...
| T24.10 | ✅ Fatto | **Layout pluggabili e plancia LCARS**: la struttura dello schermo diventa un plug-in (`TuiLayoutEngine`, `registerLayoutEngine`, campo `engine`); motori di serie `classic` e `console` (plancia TNG: colonna di pulsanti F1–F12, un gomito che incornicia conversazione e prompt, lettura dell'agente, colonna dati con widget di attività e file). Mouse e focus leggono le regioni del frame disegnato; Tab salta i pannelli assenti (prima finiva su pannelli invisibili, anche nel preset zen). Default: console LCARS. |
| T24.11 | ✅ Fatto | **Suggerimenti nel prompt della TUI**: menu sopra il prompt con comandi, agenti, team, ruoli, tratti, provider, livelli di effort e `@menzioni`; ↑/↓ scelgono, Tab completa, Esc chiude, Invio invariato. Una sola tabella dati `ARGUMENTS` (`src/cli/commands/completion.ts`) condivisa da CLI e TUI al posto di due catene di `if` divergenti (la TUI non completava `/agent`); rimossa la lista morta di comandi in `Input.ts`. |
| T24.12 | ✅ Fatto | **A capo automatico nel prompt della TUI**: il prompt divideva il testo solo sugli a capo veri e troncava ogni riga al bordo, quindi scrivendo a lungo testo e cursore sparivano; anche l'altezza contava solo i `
| T24.13 | ✅ Fatto | **`web_search` bloccato in silenzio e risposta del server invisibile**: DuckDuckGo risponde HTTP 202 con una pagina anti-bot ("anomaly"); il backend trattava ogni 2xx come successo, il parser non trovava nulla e il modello riceveva "No useful web search results found". Ora solo 200 porta risultati, ogni altro status è un errore esplicito con il suggerimento di cambiare provider (`/search-engine`). Il contratto `WebSearchBackend.search(query, onTrace?)` riporta lo scambio grezzo (richiesta con credenziali oscurate, status, content-type, byte, risultati, corpo fino a `webSearchTraceMaxChars`) tramite il nuovo evento `tool_diagnostics`, che la vista Tools mostra sotto la chiamata come sezione "server" e che non entra mai nella conversazione. Test WS.12–WS.15. |
| T24.14 | 🔲 Da fare | **Provider di ricerca senza chiave e locali**: DuckDuckGo HTML blocca le richieste automatiche da alcuni indirizzi; gli altri provider di serie richiedono una chiave. Valutare un'istanza SearXNG locale (JSON), che oggi la policy di rete rifiuta perché su loopback: serve l'allowlist esplicita di servizi locali prevista da T23.7 e non ancora implementata. |
| T24.15 | ✅ Fatto | **Google tolto dal catalogo di ricerca**: la Custom Search JSON API è chiusa ai nuovi clienti dal 2025 e viene spenta il 1° gennaio 2027 (il sostituto Vertex AI Search cerca solo su siti scelti). Restano `duckduckgo` e `tavily` (piano gratuito da 1.000 crediti al mese, senza carta). La meccanica "chiave nella query string" resta coperta da un provider finto definito nel test. |
| T24.16 | ✅ Fatto | **Failover all'avvio salvato per sempre e `/reset` incompleto nella TUI**: se il provider configurato non rispondeva all'avvio (per esempio Unsloth Studio ancora in caricamento), CLI e TUI passavano a un altro provider e lo **salvavano** in `tsuka.config.json`, così alle aperture successive TSUKA restava sul cloud. Ora il failover vale solo per il processo (`ConfigManager.useProviderForSession`, statico perché le istanze sono molte); la scelta salvata resta e una scelta esplicita con `/provider` cancella l'override. La `/reset` della TUI non azzerava `ContextTracker` né i contatori dell'header (la CLI sì): funzione condivisa `resetSessionState` in `src/core/sessionReset.ts`. Suite `test_session_reset_failover.ts` (7 check). |
| T24.17 | ✅ Fatto | **Unsloth scavalcato dal cloud a ogni avvio**: la prima richiesta a Unsloth Studio dopo un periodo di inattività impiega circa 2,5 s (poi 0,2 s), esattamente il timeout della sonda di avvio; la sonda falliva e il failover passava a OpenRouter (finestra 512k). Il provider configurato ora ha `DISCOVERY_DEFAULTS.configuredProbeTimeoutMs` (10 s); i candidati di riserva restano a 2,5 s. Test PF10 con un server che risponde dopo 3 s e rispetta l'abort. |
| T24.18 | ✅ Fatto | **Potatura senza margine**: la storia veniva potata fino al 100% della finestra in token *stimati*; il server ne contava di più e rifiutava la richiesta (checkpoint A: 12596 reali contro 12544). Ora la potatura lascia libero `AGENT_DEFAULTS.historyHeadroomRatio` (10%) per errore di stima e risposta; la pressione resta misurata sull'intera finestra. Test T2.3f. |
| T24.19 | ✅ Fatto | **CI rossa su Node 20 e modello di avvio**: `download_file` lasciava il `.part` perché l'unlink correva contro l'apertura dello stream (ora attende la chiusura); `test_self_authoring` verifica il rifiuto fail-closed sotto Node 25 invece di contarlo come errore; matrice CI con Node 26. Regola unica `chooseStartupModel` (RAM, poi config se elencato, poi primo elencato) per CLI, TUI e runner del checkpoint. Test PF11. |
`. Nuovo `src/tui/promptWrap.ts` (a capo visuale tra le parole, offset nel testo originale per il cursore, caratteri larghi misurati in colonne); `computeInputHeight` conta le righe visuali per la larghezza del pannello, il prompt cresce fino al massimo e poi scorre seguendo il cursore. Invio invia sempre il testo come una riga; Shift/Ctrl+Invio restano l'a capo vero. Suite `test_prompt_wrap.ts` (13 check). |
...
| Budget del prompt | `selectForPrompt` nel contratto, rendering nel facade | `facade.ts` (T24.4) | Risolto in T24.4: il budget è applicato una volta per tutti i backend |
...
| Superficie minima | `remove` | il facade espone `forgetFact` per il tool | Risolto in T24.4 |
...
**Priorità:** media · **Esecutore:** maintainer, con il proprio server LLM · **Stato:** fatto il 26/09/2026, esito in Tappa 12.7 delle guide didattiche

**Esecuzione:** `npx tsx scripts/checkpoint-a.ts [--reps N] [--tasks A,B,C]` lavora in un worktree usa e getta, rifiuta la shell e scrive `runs/checkpoint-a/<data>/results.md` e `answers.md`. Per far scattare la delega serve una finestra piccola (12–16k): con finestre realistiche la pressione resta sotto 0,70.
...
**Colonna dati (aggiunta su richiesta del maintainer, stesso giorno):** la prima versione della console non mostrava né l'esplorazione file né i widget di attività (LED di ragionamento/generazione/tool, attività dei tool, metriche). Ora, da `consoleSideMinWidth` (110 colonne) in su, la console ha a destra una colonna dati separata da una barra verticale: il pannello OPERATIONS con i widget di `visibleWidgets` (esclusa la persona, già nel blocco AGENTE), a tutta altezza. Sotto quella larghezza la colonna sparisce per non schiacciare la conversazione. L'elenco dei file sta nella colonna di sinistra, sotto i pulsanti F1–F12 (su indicazione del maintainer), così resta visibile anche sui terminali stretti; è nascondibile da F7 e sparisce se l'altezza non basta a mostrare `minFilesHeight` righe. Il codice `47-1701` sulla barra di mezzo è decorativo (`code` del tema LCARS). Test: `tests/test_tui_layout.ts` (plug-in registrato e usato, fallback, pulsanti cliccabili sulle righe disegnate, larghezze esatte per ogni motore, tema e larghezza).

## T24.11 — Suggerimenti nel prompt della TUI

**Stato:** ✅ Fatto (2026-09-26) · **Priorità:** media

**Richiesta del maintainer:** scrivendo `/agent ge` la TUI deve proporre Geordi e permettere di completarlo con frecce o Tab; lo stesso per i team.

**Problema trovato:** il completamento degli argomenti era scritto due volte come catena di `if`: in `src/cli/index.ts` (con `/agent`) e in `src/tui/inputCompletion.ts` (senza `/agent`, `/role`, `/trait`, `/provider`, `/effort`…), più una terza lista di comandi in `Input.ts` che nessuno usava. Nessun elenco visibile: Tab completava alla cieca e i candidati multipli finivano in una notifica.

**Intervento:** tabella dati `ARGUMENTS` in `src/cli/commands/completion.ts` (comando → fornitore di `CompletionItem` con valore, suggerimento e alias), usata da CLI e TUI; gli agenti si filtrano anche per nome completo. Nella TUI `CompletionMenu` (`interaction/completionMenu.ts`) calcola i suggerimenti, con memoria sullo stato dell'input, e gestisce ↑/↓/Tab/Esc solo quando il menu è aperto; Invio resta sempre l'invio del testo scritto. La selezione vale solo per lo stato dell'input su cui è stata fatta (`completion.key`), quindi ogni modifica riapre il menu sul primo candidato. Il menu (`views/CompletionMenu.ts`) è una sovrapposizione disegnata in `composeLayoutFrame` sopra il rettangolo del prompt riportato dal motore di layout: funziona con console, classic e futuri plug-in senza che i motori lo conoscano. Il tipo `CompletionMenuView` sta nel contratto dei layout (evita un ciclo di import). Test: `tests/test_tui_input_completion.ts` (17 check).
