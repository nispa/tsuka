...
async function main() {
  ...
  // T1: felice — STATO: COMPLETATO al primo turno → early stop, secondo membro mai chiamato
  {
    ...
    check('RR1b', provider.remaining === 0, 'script consumato esattamente 1 volta: piccione non è mai stato chiamato dopo il COMPLETATO di falco');
  }
  ...
  {
    const provider = new MockLLMProvider([
      { content: 'AGENTE: @falco' },                    // decisione dell'orchestrator
      { content: 'Fatto.\nSTATO: COMPLETATO' }           // turno di falco
    ]);
    const ctx = buildMockCtx(provider);
    const team = { members: ['overseer', 'falco', 'piccione'], orchestrator: 'overseer' };
    const interrupt = new GenerationInterrupt();
    const r = await runOrchestrated(ctx, team, 'analizza i log', 2, interrupt, seedTeamMessages('analizza i log'));
    check('OR1', r.completed === true && r.roundsDone === 1, `routing verso @falco seguito e completato (completed=${r.completed}, roundsDone=${r.roundsDone})`);
  }
  ...
  // T4: rottura — risposta dell'orchestrator non parseabile → fallback round-robin, con warning visibile
  {
    const provider = new MockLLMProvider([
      ...
      { content: 'Ok, procedo io.\nSTATO: COMPLETATO' }               // turno del fallback (falco)
    ]);
    const ctx = buildMockCtx(provider);
    const team = { members: ['overseer', 'falco'], orchestrator: 'overseer' };
    const interrupt = new GenerationInterrupt();
    ...
    check('OR2b', r.completed === true, `il fallback ha comunque chiamato @falco (unico worker) e completato (completed=${r.completed})`);
  }
  ...
  {
    const provider = new MockLLMProvider([
      { content: 'Ho iniziato il lavoro.\nSTATO: DA_CONTINUARE' }, // stazione 1: falco
      { content: 'Ho finito tutto.\nSTATO: COMPLETATO' }           // stazione 2: piccione
    ]);
    const ctx = buildMockCtx(provider);
    const team = { members: ['falco', 'piccione'] };
    const interrupt = new GenerationInterrupt();
  ...
  {
    const provider = new MockLLMProvider([
      { content: 'Ho lavorato.\nSTATO: DA_CONTINUARE' },     // turno di lavoro di falco
      { content: '"Ottimo lavoro."\nVOTO: APPROVO' }          // discussione/voto di falco
    ]);
    const ctx = buildMockCtx(provider);
    const team = { members: ['falco'], discussionRounds: 1, voting: true };
    const interrupt = new GenerationInterrupt();
  ...
  // RRT1: round-robin, felice — report_status(COMPLETATO) via tool call, nessun marker testuale
  {
    ...
    check('RRT1b', provider.remaining === 0, 'piccione non è mai stato chiamato: la tool call ha chiuso il turno di falco, non un marker testuale');
  }
  ...
  // ORT1: orchestrated, felice — route_next(@falco) via tool call, nessun marker AGENTE: testuale
  {
    const provider = new MockLLMProvider([
      { toolCalls: [mockToolCall('route_next', { agent: 'falco', reason: 'È il più adatto al compito.' })] }, // decisione orchestrator via tool
      { content: 'Fatto.\nSTATO: COMPLETATO' } // turno di falco
    ]);
    const ctx = buildMockCtx(provider);
    const team = { members: ['overseer', 'falco', 'piccione'], orchestrator: 'overseer' };
    const interrupt = new GenerationInterrupt();
    const r = await runOrchestrated(ctx, team, 'analizza i log', 2, interrupt, seedTeamMessages('analizza i log'));
    check('ORT1', r.completed === true && r.roundsDone === 1, `routing via tool call route_next verso @falco seguito e completato (completed=${r.completed}, roundsDone=${r.roundsDone})`);
  }

  // ORT2: orchestrated, rottura — solo testo (AGENTE: @nome) → fallback a regex, degrado segnalato
  {
    const provider = new MockLLMProvider([
      { content: 'AGENTE: @falco' },
      { content: 'Fatto.\nSTATO: COMPLETATO' }
    ]);
    const ctx = buildMockCtx(provider);
    const team = { members: ['overseer', 'falco'], orchestrator: 'overseer' };
    ...
    check('ORT2a', r.completed === true, `nessuna tool call route_next: fallback a regex AGENTE: @falco → comunque instradato (completed=${r.completed})`);
  ...
  // PLT2: pipeline — report_status(FALLITO) via tool call interrompe la catena (gap T1.2/PLANNING.md ora chiuso)
  {
    ...
    check(
      ...
      `[T2.1] STATO: FALLITO via report_status ora ferma la pipeline (gap PLANNING.md chiuso): completed=${r.completed}, failed=${r.failed}, roundsDone=${r.roundsDone}, piccione mai chiamato`
    );
  }
  ...
  // PLT4: pipeline con acceptance e retry loop (T6.4) — tent. 1 fallisce (FALLITO), tent. 2 passa
  {
    ...
    const ctx = buildMockCtx(provider);
    const team = { members: ['falco'], acceptance: { fileExists: 'README.md' }, maxAttempts: 2 };
    const interrupt = new GenerationInterrupt();
