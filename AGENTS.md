...
* **Metrics**: 30 native tools · 24 characters/agents · 21 roles · 9 traits · 10 preconfigured teams · 20 REPL slash commands · 91 automated test suites · Dual CLI & TUI Interactive Interfaces.
...
6. **No Test Regressions**: All 91 test suites (`npm test`) must pass cleanly before completing any task. Automated tests must use mock stores and temporary test directories—never mutate the active user's `memory.json`.
...
10. **Small, Sharp, and Explain the Why**: Keep implementations focused, minimal, and easy to understand. Do not settle for the first working patch when a smaller coherent design removes the cause. Do not introduce fragile special cases, dead code, speculative abstractions, duplicated policy, or complexity disproportionate to the behavior being implemented. Comment non-obvious inference mechanics, cache and process lifetimes, memory policy, protocol ordering, safety boundaries, and API orchestration beside the implementation. Comments must be compact and instructive: explain why a shape, ordering, ownership boundary, or recovery path exists, not what the syntax already says. Keep public contracts narrow: CLI and TUI depend on core contracts and events, never on provider wire formats, storage internals, MCP process details, or other backend-specific state. Prefer one production path. Diagnostic switches are allowed when they observe or validate that path; do not preserve permanent semantic variants behind flags.
...
│   │   ├── shared.ts                # Compatibility barrel for the core persona subsystem
...
│   │   ├── personas.ts              # Persona catalog, asset loading, resolution, and system-prompt assembly
│   │   ├── workflowDispatcher.ts     # Narrow application boundary used by workflow escalation tools
...
│   │   │                            #   wire format payload building, stream accumulator, error mapping,
│   │   │                            #   provider failure logging (providerLogger.ts); `ILLMProvider` contract
│   │   ├── types.ts                 # Core protocol interfaces & shared types
│   │   ├── contextBudget.ts         # Token estimations, runtime calibration, capForContext
│   │   ├── tokenCalibration.ts      # Pure runtime characters-per-token calibration state
│   │   ├── conversationHistory.ts   # Mutable history ownership, system invariant, and pruning
│   │   ├── toolRound.ts              # Ordered tool-call execution and tool lifecycle events
│   │   ├── reasoningTrace.ts         # Persisted reasoning artifacts and memory pointers
│   │   ├── reactState.ts             # ReAct text/tool/round state transitions
...
│   │   │                            #   default JsonMemoryBackend (JSON+BM25), codec in codec.ts,
│   │   │                            #   atomic I/O in storage.ts, scoring in bm25.ts, eviction in retention.ts
...
│   │   ├── cloudProvider.ts         # ProviderClass normalization (LOCAL / CLOUD)
│   │   ├── providerCatalog.ts        # Data-driven provider definitions and capability validation
...
│   │   ├── runtime.ts               # Shared composition root & lifecycle manager (createHarnessRuntime)
│   │   ├── mcp/                     # Native MCP (Model Context Protocol) stdio client: wire
│   │   │                            #   types & IMcpClient contract, StdioTransport JSON-RPC,
│   │   │                            #   client handshake, registry adapter, server connector
...
│   │   ├── types.ts                 # Tool contracts, IToolRegistry, execution context, descriptors
│   │   ├── schema.ts                # Schema resolution, caching, and parameter validation
...
├── tests/                           # 91 automated test suites
...
### 6. Data-Driven LLM Providers & Separation of Concerns
* **Declarative Catalog (`providers.json`)**: Providers (Ollama, OpenRouter, Unsloth, Bailu, etc.) are declared with endpoint, default model, `LOCAL`/`CLOUD` classification, environment variable for API keys, and optional capabilities (e.g. `freeModels`).
* **Isolation of Overrides (`tsuka.config.json`)**: Workspace-level configuration only chooses the active provider and overrides specific models/endpoints when needed.
* **Smart Probing & Tier Policy**: Local servers trigger context-window auto-detection and RAM model warmup warnings; cloud providers automatically inherit tier `LARGE` by policy.
...
# Execute full automated test suite (91 test suites)
npm test
