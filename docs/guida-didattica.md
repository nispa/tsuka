...
| **Credenziali** | Chiavi API che arrivano al modello | Due barriere in `credentials.ts`: i processi figli partono senza le variabili-credenziale (eccezioni dichiarate con `commandEnvPassthrough` o `env` del server MCP), e ogni risultato di tool viene ripulito dai valori segreti noti prima di entrare nella history | Riconosce solo i segreti che TSUKA conosce: una password in un file qualsiasi è testo come un altro |
