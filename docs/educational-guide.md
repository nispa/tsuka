...
| **Credentials** | API keys reaching the model | Two barriers in `credentials.ts`: child processes start without credential variables (exceptions declared via `commandEnvPassthrough` or an MCP server's `env`), and every tool result is scrubbed of known secret values before it enters the history | Only secrets TSUKA knows are recognized: a password in some arbitrary file is just text |
