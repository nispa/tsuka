...
| **Credential redaction** | Keys ending up in a prompt or a log | `get_ps_info`'s environment listing filters sensitive names; provider logs and web-search errors redact credentials; generated tools get an empty environment | The shell and MCP servers inherit the full environment: a command can read a key |

Two rows repeat almost verbatim in the right-hand column: **the shell and MCP servers**. That is not an accident. A shell command or an external server is a program with the permissions of the process that starts it; no check on the command's text turns it into a sandbox. That is why `execute_command` stays `DANGEROUS`, MCP servers start as `RESTRICTED` unless configured otherwise, and `/sudo` is an explicit user choice.
